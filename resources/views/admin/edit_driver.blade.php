...
<div class="row">
<form action="  {{ url('admin/edit_driver/'.$bookings->id)}}" method="POST">
  {{ csrf_field() }}
  <input type="hidden" name="_method" value="PUT">
  @foreach ($bookings->drivers as $olddriver)
    ...
    <ul>
      <li>
          <select class="" name="drivers[]">
            ...
            <option value="{{ $driver->id }}"  @if($olddriver->id == $driver->id)
            selected="selected" @endif>{{ $driver->nama_driver }}</option>
            @endforeach
          </select>
    </li>
    </ul>
